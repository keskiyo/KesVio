...
$missingInstaller = Join-Path ([IO.Path]::GetTempPath()) "kesvio-missing-installer-$([Guid]::NewGuid().ToString('N')).exe"
$missingSignature = Join-Path ([IO.Path]::GetTempPath()) "kesvio-missing-signature-$([Guid]::NewGuid().ToString('N')).sig"
