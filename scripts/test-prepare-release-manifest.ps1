...
$fixtureRoot = Join-Path ([IO.Path]::GetTempPath()) "kesvio-manifest-test-$([Guid]::NewGuid().ToString('N'))"
$assetsDir = Join-Path $fixtureRoot "assets"
$notesPath = Join-Path $fixtureRoot "release-notes.md"

try {
  New-Item -ItemType Directory -Path $assetsDir | Out-Null
  [IO.File]::WriteAllBytes((Join-Path $assetsDir "KesVio_9.8.7_x64-setup.exe"), [byte[]](1, 2, 3))
  [IO.File]::WriteAllText(
    (Join-Path $assetsDir "KesVio_9.8.7_x64-setup.exe.sig"),
    "test-signature",
