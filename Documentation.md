...
only place with the primary **Refresh catalog** action and owns the
Applications, Sources and Last scan facts. The **Catalog sources** section
below it keeps the summary line and the **Source details** disclosure.
...
five-column table when the page surface is wide enough. On wide screens the
source section sits beside the narrower **Last scan** section; they stack on
smaller screens. **Last scan** owns the added, updated and removed counts and
the duration in plain numbers without repeating the total application count,
with mode, per-source and per-visibility counts, unreachable folders and the
launch-target check behind **Technical details**.
...
rules, the helper veto, the external cap at normal and the curated > external

> generated order change only for a reproduced real query, never on
> suspicion. Nothing records user queries and nothing leaves the machine; a
> developer investigates a report by hand:
