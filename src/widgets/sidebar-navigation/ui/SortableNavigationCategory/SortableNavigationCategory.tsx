...
import type { SortableNavigationCategoryProps } from '../../types'
...
export function SortableNavigationCategory({
	...
	onSelect,
}: SortableNavigationCategoryProps) {
	const {
		attributes,
		listeners,
		setActivatorNodeRef,
	...
	return (
		<button
			...
			className={`${navigationCategoryRowClass} cursor-grab touch-none transition-[background-color,border-color,box-shadow,transform] hover:bg-(--surface-raised) focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-(--accent-strong) active:cursor-grabbing motion-reduce:transition-none ${isDragPreviewActive ? 'opacity-0' : ''}`}
			{...attributes}
			{...listeners}
			ref={node => {
				setNodeRef(node)
