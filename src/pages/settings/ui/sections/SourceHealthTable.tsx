				...
				{rows.map(row => (
					...
					>
						<div className="flex min-w-0 items-start justify-between gap-3">
							...
							<span
								className={`shrink-0 text-xs font-medium ${row.needsAttention ? 'text-amber-800' : 'text-slate-600'}`}
							>
								{row.statusLabel}
							</span>
						</div>
				...
				<tbody>
					{rows.map(row => (
						...
						>
							...
							<td
								className={`py-1.5 pr-3 break-words ${row.needsAttention ? 'font-medium text-amber-800' : ''}`}
							>
								{row.statusLabel}
							</td>
...
function lastSuccessText(lastSuccessAt: number | null): string {
	return lastSuccessAt
		? timestampFormatter.format(new Date(lastSuccessAt * 1000))
		: 'Never'
}
