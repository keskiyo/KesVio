 ...
 * payload whose first byte is 0x03 — is what puts AppNook on that page already switched off, ready
...
describe('NSIS startup registration', () => {
	...
	it('registers that shortcut as disabled rather than switching autostart on', () => {
		...
		expect(install).toMatch(
			/WriteRegBin HKCU "\$\{APPNOOK_STARTUP_APPROVED\}" "\$\{PRODUCTNAME\}\.lnk" "030{22}"/,
		)
 ...
 * non-recursive `RMDir "$INSTDIR"`. None of that reaches $INSTDIR\AppNookData, which is where
...
describe('NSIS data-root cleanup', () => {
	...
	it('deletes the data root only when the user asked for it', () => {
		...
		expect(hookBody).toContain('RMDir /r "$INSTDIR\\AppNookData"')
	})

	it('always removes the diagnostics logs, which are not user data', () => {
		expect(hookBody).toContain('RMDir /r "$INSTDIR\\AppNookData\\logs"')
	})
	...
	it('never touches the data root while updating', () => {
		...
		expect(guard).toBeGreaterThan(-1)
		expect(hookBody.indexOf('AppNookData')).toBeGreaterThan(guard)
	})
