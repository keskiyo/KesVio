...
import { ROW_CHIP } from '../../../src/pages/settings/data'
import {
	ACTION_BUTTON,
	ACTION_BUTTON_NEUTRAL,
	ACTION_BUTTON_PRIMARY,
	ACTION_BUTTON_QUIET,
} from '../../../src/shared/ui/buttonVariants'
 ...
 * `ACTION_BUTTON`, which is the shape the card actions below already used; the Catalog Health and
 * Backup & Restore pages under More reuse the same shared shape instead of a copy.
 */
describe('settings row controls', () => {
	...
	it('keeps the card actions on the shared shape rather than a copy of it', () => {
		for (const source of cardActions) {
			...
			expect(source).not.toContain('border border-slate-300 bg-slate-100')
			expect(source).toMatch(/ACTION_BUTTON_(?:PRIMARY|NEUTRAL)/)
		}
	})
})
