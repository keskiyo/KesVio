...
pub(super) static RULES: &[Rule] = &[
    ...
    rule(Utilities, Name, NAME, &["goosedesktop"]),
    rule(Utilities, Path, PATH, &[r"\desktop goose"]),
    rule(
        Productivity,
        Name,
        NAME,
        &["поликлиника", "здравоохранение", "anthroplus"],
    ),
    rule(Editors, Publisher, PUBLISHER, &["medixant"]),
    rule(Editors, Name, NAME, &["dicom"]),
    rule(Media, Publisher, PUBLISHER, &["devline"]),
    rule(Media, Name, NAME, &["cctv"]),
    rule(
        System,
        Desc,
        DESC,
    ...
    rule(System, ExeContains, EXE_GLUE, &["bootice"]),
    rule(Utilities, Desc, DESC, &["storage performance", "rtss"]),
    rule(Utilities, Product, PRODUCT, &["smart wifi solution"]),
    rule(
        Development,
        Path,
        PATH,
        &[r"\mingw64\bin\", r"\git\usr\bin\"],
    ),
    rule(Development, Publisher, PUBLISHER, &["cloudbees"]),
    rule(Media, Name, NAME, &["x264vfw", "animewatch"]),
    rule(Editors, Publisher, PUBLISHER, &["аскон"]),
    rule(Editors, Path, PATH, &[r"\kompas-3d"]),
    rule(Utilities, ExeContains, EXE_GLUE, &["desktoppet"]),
    rule(Utilities, Name, NAME, &["desktoplara"]),
